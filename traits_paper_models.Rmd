...
#Functional richness #something is wrong with the Fric values - recalculate and reimport.  it's meant to vary between 0 and 1 (therefore should be binomial).  higher is richer
...
for (i in 1:rep) {
  ...
  FDtemp<-dbFD(pref_traits, temp, stand.x=FALSE, ord="podani", corr = "cailliez") #this is silly I don't need FD in here, I need something like NND and maxD after Mouillot et al.  but maybe this will tell me something else 
...
require(FD)
rep=99
array<-array(0, dim=rep,4,24)
for (i in 1:rep) {
  temp<-randomizeMatrix(cover, null.model="independentswap", iterations = 1000)
  FDtemp<-dbFD(pref_traits, temp, stand.x=FALSE, ord="podani", corr = "cailliez") #this is silly I don't need FD in here, I need something like NND and maxD after Mouillot et al.  but maybe this will tell me something else #maybe make an array and save the value for each community and test each community against its own null distribution??
  for (j in 1:24){
  array [i, ] <-c(FDtemp[j,FEve], FDtemp[j,FDiv], FDtemp[j,FDis], FDtemp[j,RaoQ])
  }
}
