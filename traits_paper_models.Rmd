...
#rough imputation to allow ordination to proceed  - give any NA the median value for that column
fx=function(x){
...
Comparison of functional diversity metrics with the preferred traits for the study
```{r}
...
require(lme4)
FDivcsv<-read.csv("FDiv.csv", header=TRUE)
FDivcsv<-FDivcsv[,1:4]

FDivtest<-lm(FDiv ~ Stream/Site/Bank, data=FDivcsv)
anova(FDivtest)

#OR to conform to other tests in the slopes paper
require(arm)
fdiv0<-glm(FDiv ~ 1, data=FDivcsv)
fdiv1<-glm(FDiv ~Stream, data=FDivcsv)
fdiv2<-glm(FDiv ~ Stream/Site, data=FDivcsv)
fdiv3<-glm(FDiv ~ Stream/Site/Bank, data=FDivcsv)
display(fdiv0)
display(fdiv1)
display(fdiv2)
display(fdiv3)
...
```
------------------------------------------------------------------------------------
*Native vs. introduced portions of the community* (2013 data)
...
#posthoc tests site by site - null models
require(picante)
require(FD)
rep=99
permCWM<-matrix(nrow=rep, ncol=53)
for (i in 1:rep) {
  temp<-randomizeMatrix(int_partcomm, null.model="independentswap", iterations = 1000)
...
#posthoc tests of individual traits
#these all use grouping=type as defined in the overall ordination above (53 data points for inv, 53 for nat)

##leafc
leafc<-as.data.frame(int_partcomm[,1])
anosim(leafc,type,distance="gower")

##leafn
leafn<-as.data.frame(int_partcomm[,5])
anosim(leafn,type,distance="gower")

##leafp
leafp<-as.data.frame(int_partcomm[,7])
anosim(leafp,type,distance="gower")

##leafk
leafk<-as.data.frame(int_partcomm[,9])
anosim(leafk,type,distance="gower")

##cn
cn<-as.data.frame(int_partcomm[,2])
anosim(cn,type,distance="gower")

##np
np<-as.data.frame(int_partcomm[,6])
anosim(np,type,distance="gower")

##sla
sla<-as.data.frame(int_partcomm[,11])
anosim(sla,type,distance="gower")

##lwr
lwr<-as.data.frame(int_partcomm[,3])
anosim(lwr,type,distance="gower")

##stomata
stomata<-as.data.frame(int_partcomm[,20])
anosim(stomata,type,distance="gower")

##ssd
ssd<-as.data.frame(int_partcomm[,19])
anosim(ssd,type,distance="gower")

#woodymass
woodymass<-as.data.frame(int_partcomm[,21])
anosim(woodymass,type,distance="gower")

##rootn
rootn<-as.data.frame(int_partcomm[,14])
anosim(rootn,type,distance="gower")

##rootdepth
rootdepth<-as.data.frame(int_partcomm[,15])
anosim(rootdepth,type,distance="gower")

##seedno
seedno<-as.data.frame(int_partcomm[,17])
anosim(seedno,type,distance="gower")

##protein
protein<-as.data.frame(int_partcomm[,18])
anosim(protein,type,distance="gower")

##seedmass
seedmass<-as.data.frame(int_partcomm[,16])
anosim(seedmass,type,distance="gower")

##rgr
rgr<-as.data.frame(int_partcomm[,13])
anosim(rgr,type,distance="gower")

##photosynth
ps<-as.data.frame(int_partcomm[,12])
anosim(ps,type,distance="gower")

##resp
resp<-as.data.frame(int_partcomm[,10])
anoresp<-anosim(resp,type,distance="gower")

```



#look at pairwise differences within each site (unfinished code)
```{r}
...
#adonis and vegan can't do pairwise tests
#can i do it by hand the way I did for the individual traits above?
#e.g.

pwtype=c(rep("Introduced", 1), rep("Native", 1))
Alplaus<-as.data.frame(int_partcomm[c(1,54),])
anoalplaus<-anosim(Alplaus,pwtype,distance="gower")
```
...
testpca<-princomp(diff)
biplot(testpca)


#this is the bit that works for plotting traits on things
require(ade4)
...
another attempt at pairwise using betadisper

```{r}
bdiff<-vegdist(int_partcomm, method="gower")
groups<-factor(c(rep(1,53), rep(2,8)), labels=c("introduced", "native"))
mod<-betadisper(bdiff, groups)
```


Trying to get pairwise significance values from adonis  - ignore.  this loop works but doesn't give me what i want it to give me, adonis doesn't give a significance on differences between groups, it partitions differences based on each variable
...
for (i in 1:rep) {
  temp<-diff[c(i,j),]
  compare<-adonis(diff~Leaf.C.per.dry.mass + Leaf.C.to.N.ratio + Leaf.weight.ratio + Lamina.thickness + Leaf.N.per.dry.mass + Leaf.N.to.P.ratio + Leaf.P.per.dry.mass + Leaf.physical.strength + Leaf.K.per.dry.mass + Leaf.K.per.dry.mass + Leaf.respiration.per.dry.mass + SLA + Photosynthesis.per.leaf.dry.mass + RGR + Root.N.per.dry.mass + Rooting.depth + Seed.mass + Seed.number.per.reproduction.unit + Seed.protein.content.per.mass + SSD + Stomata.conductance.per.leaf.area + Wood.dry.mass.per.total.dry.mass, permutations=99, int_partcomm, method="gower")
  pairwise[[i]]=compare
  j=j+1
}
...
#rough imputations to allow ordination to proceed  - give any NA the median value for that column
fx=function(x){
  x[is.na(x)]=median(x, na.rm=TRUE)
  x
}
...
```
TODAY
look at different lifespans

```{r}


#TRAITS
nat_ann<-read.csv("2013_nattraits_impute_annual.csv", row.names=1)
nat_per<-read.csv("2013_nattraits_impute_perennial.csv", row.names=1)
inv_ann<-read.csv("2013_invtraits_impute_annual.csv", row.names=1)
inv_per<-read.csv("2013_invtraits_impute_perennial.csv", row.names=1)
#biennials were only seen for invasives and not natives, so no point in ordinating
#FD requires species as rows and traits as columns


#subset traits (FD won't work with empty trait columns
natann<-Filter(function(x)!all(is.na(x)), nat_ann)
natper<-Filter(function(x)!all(is.na(x)), nat_per)
invann<-Filter(function(x)!all(is.na(x)), inv_ann)
invper<-Filter(function(x)!all(is.na(x)), inv_per)

#COMMUNITIES
nat_ann_c<-read.csv("2013_presences_nat_ann.csv")
nat_per_c<-read.csv("2013_presences_nat_per.csv")
inv_ann_c<-read.csv("2013_presences_inv_ann.csv")
inv_per_c<-read.csv("2013_presences_inv_per.csv")

#Make Community Matrices
require(reshape)

comm_natann<-cast(nat_ann_c, River ~ Species, value="Presence", fun.aggregate=mean)
comm_natann<-as.data.frame(comm_natann)
comm_natann[is.na(comm_natann)]<-0
row.names(comm_natann)<-comm_natann$River
comm_natann<-comm_natann[,2:ncol(comm_natann)]

comm_natper<-cast(nat_per_c, River ~ Species, value="Presence", fun.aggregate=mean)
comm_natper<-as.data.frame(comm_natper)
comm_natper[is.na(comm_natper)]<-0
row.names(comm_natper)<-comm_natper$River
comm_natper<-comm_natper[,2:ncol(comm_natper)]

comm_invann<-cast(inv_ann_c, River ~ Species, value="Presence", fun.aggregate=mean)
comm_invann<-as.data.frame(comm_invann)
comm_invann[is.na(comm_invann)]<-0
row.names(comm_invann)<-comm_invann$River
comm_invann<-comm_invann[,2:ncol(comm_invann)]

comm_invper<-cast(inv_per_c, River ~ Species, value="Presence", fun.aggregate=mean)
comm_invper<-as.data.frame(comm_invper)
comm_invper[is.na(comm_invper)]<-0
row.names(comm_invper)<-comm_invper$River
comm_invper<-comm_invper[,2:ncol(comm_invper)]

#Calculate CWMs
require(FD)

nafd<-dbFD(natann, comm_natann, stand.x=TRUE, ord="podani", corr="cailliez", w.abun=TRUE, stand.FRic=TRUE) #works
npfd<-dbFD(natper, comm_natper, stand.x=TRUE, ord="podani", corr="cailliez", w.abun=TRUE, stand.FRic=TRUE) #works
iafd<-dbFD(invann, comm_invann, stand.x=TRUE, ord="podani", corr="cailliez", w.abun=TRUE, stand.FRic=TRUE) #works
ipfd<-dbFD(invper, comm_invper, stand.x=TRUE, ord="podani", corr="cailliez", w.abun=TRUE, stand.FRic=TRUE) #works

#Export CWMs for safekeeping
write.table(nafd$CWM, "nafdCWM.csv", sep=",", col.names=NA, row.names=TRUE)
write.table(npfd$CWM, "npfdCWM.csv", sep=",", col.names=NA, row.names=TRUE)
write.table(iafd$CWM, "iafdCWM.csv", sep=",", col.names=NA, row.names=TRUE)
write.table(ipfd$CWM, "ipfdCWM.csv", sep=",", col.names=NA, row.names=TRUE)


#merge the native and introduced bits for herbs, and separately for woody,  reimport the site by trait matrices, listing communities separately as introduced and native portions of each site.  

anncomm<-read.csv("2013_CWM_annuals_partitionedbyorigin.csv", row.names=1)
percomm<-read.csv("2013_CWM_perennials_partitionedbyorigin.csv", row.names=1)
...
ianncomm=data.frame(apply(anncomm,2,fx))
row.names(ianncomm)<-rownames(anncomm)
ipercomm=data.frame(apply(percomm,2,fx))
row.names(ipercomm)<-rownames(percomm)
...
require(vegan)
annord<-metaMDS(ianncomm, distance="gower", noshare=FALSE, zerodist="add", k=3)
ordiplot(annord, type="n")
atype=c(rep("Introduced", 9), rep("Native", 32))
#ordihull(annord, groups=wtype, draw="polygon", col="grey90", label=F) #for tightest convex hull containing all points in the group
ordiellipse(annord, groups=atype, draw="lines", kind="sd", col="black", label=T) #for ellipse representing SD of community
points(annord, display="sites", col=c(rep("blue", 9), rep("red", 32)), pch= 19)
#there was previously in error in the code designating the colors for these points - it didn;'t match the numbers for introduced and native comms - it was 9 and 15 instead of 9 and 32, so the (correct) ellipses did not match the colors

#orditorp(annord, display="species", col="black", pch=8) #this will plot the traits ("species" in vegan package lingo which always means whatever your columns are)

##perennial

perord<-metaMDS(ipercomm, distance="gower", noshare=FALSE, zerodist="add", k=3)
ordiplot(perord, type="n")
ptype=c(rep("Introduced", 52), rep("Native", 53))
#ordihull(ord, groups=type, draw="polygon", col="grey90", label=F) #for tightest convex hull containing all points in the group
ordiellipse(perord, groups=ptype, draw="lines", kind="sd", col="black", label=T) #for ellipse representing SD of community

points(perord, display="sites", col=c(rep("red", 52), rep("blue", 53)), pch= 19)

orditorp(ord, display="species", col="black", pch=8) #this will plot the traits ("species" in vegan package lingo which always means whatever your columns are)

#test significance
anosim(ipercomm, ptype, distance="gower")
anosim(ianncomm, atype, distance="gower")


#subset leaf nitrogen (this works but is a demo. would want to do it with full data set, not partitioned)
iperleafC<-as.data.frame(ipercomm[,1])

anosim(iperleafC,ptype,distance="gower")
```
...
plot the introduced versus native traits (all traits, no imputed values)
```{r}
invtraits<-read.csv("2013_traits_invs.csv")
nattraits<-read.csv("2013_traits_natives.csv")
...
names(alltraits2)[names(alltraits2)=="Leaf carbon (C) content per dry mass"] <-"leafC" #ddply doesn't seem to like spaces in column names, or ""ing them
...
ggplot(cleafC, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf C per dry mass (mg/g)")
...
ggplot(cleafN, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Leaf N per dry mass (mg/g)")

#Leaf P

names(alltraits2)[names(alltraits2)=="Leaf phosphorus (P) content per dry mass"] <-"leafP" #ddply doesn't seem to like spaces in column names, or ""ing them

cleafP<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(leafP)),
             mean = mean(leafP, na.rm=TRUE),
             sd = sd(leafP, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cleafP, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Leaf P per dry mass (mg/g)")

#Leaf K

names(alltraits2)[names(alltraits2)=="Leaf potassium (K) content per dry mass"] <-"leafK" #ddply doesn't seem to like spaces in column names, or ""ing them

cleafK<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(leafK)),
             mean = mean(leafK, na.rm=TRUE),
             sd = sd(leafK, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cleafK, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Leaf K per dry mass (mg/g)")

#C:N
names(alltraits2)[names(alltraits2)=="Leaf carbon/nitrogen (C/N) ratio"] <-"leafCN" #ddply doesn't seem to like spaces in column names, or ""ing them

cleafCN<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(leafCN)),
             mean = mean(leafCN, na.rm=TRUE),
             sd = sd(leafCN, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cleafCN, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Leaf C:N ratio")

#N:P

names(alltraits2)[names(alltraits2)=="Leaf nitrogen/phosphorus (N/P) ratio"] <-"leafNP" #ddply doesn't seem to like spaces in column names, or ""ing them

cleafNP<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(leafNP)),
             mean = mean(leafNP, na.rm=TRUE),
             sd = sd(leafNP, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cleafNP, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Leaf N:P ratio")
...
names(alltraits2)[names(alltraits2)=="Leaf dry mass per plant dry mass (leaf weight ratio, LWR)"] <-"lwr" #ddply doesn't seem to like spaces in column names, or ""ing them

clwr<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(lwr)),
             mean = mean(lwr, na.rm=TRUE),
             sd = sd(lwr, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(clwr, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("LWR")

#Stomata conductance

names(alltraits2)[names(alltraits2)=="Stomata conductance per leaf area"] <-"stomataconductance" #ddply doesn't seem to like spaces in column names, or ""ing them

cstomataconductance<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(stomataconductance)),
             mean = mean(stomataconductance, na.rm=TRUE),
             sd = sd(stomataconductance, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cstomataconductance, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Stomata conductance per leaf area")
...
names(alltraits2)[names(alltraits2)=="Wood dry mass per plant dry mass"] <-"woodymass" #ddply doesn't seem to like spaces in column names, or ""ing them

cwoodymass<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(woodymass)),
             mean = mean(woodymass, na.rm=TRUE),
             sd = sd(woodymass, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cwoodymass, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Woody dry mass per plant dry mass")

#RootN

names(alltraits2)[names(alltraits2)=="Root nitrogen (N) content per dry mass"] <-"RootN" #ddply doesn't seem to like spaces in column names, or ""ing them

crootN<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(RootN)),
             mean = mean(RootN, na.rm=TRUE),
             sd = sd(RootN, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(crootN, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Root N (mg per g dry root mass)")


#Rootdepth

names(alltraits2)[names(alltraits2)=="Rooting depth"] <-"rootdepth" #ddply doesn't seem to like spaces in column names, or ""ing them

crootdepth<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(rootdepth)),
             mean = mean(rootdepth, na.rm=TRUE),
             sd = sd(rootdepth, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(crootdepth, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Root depth (cm)")

#Seednumber

names(alltraits2)[names(alltraits2)=="Seed number per reproducton unit"] <-"seedno" #ddply doesn't seem to like spaces in column names, or ""ing them

cseedno<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(seedno)),
             mean = mean(seedno, na.rm=TRUE),
             sd = sd(seedno, na.rm=TRUE),
             se = sd/sqrt(N))


cseedno$Origin<-factor(cseedno$Origin, levels=c("N", "I")) #puts the natives on the lefthand side

ggplot(cseedno, aes(x=Origin, y=log(mean,10))) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=log(mean-se,10), ymax = log(mean+se,10), width=.2)) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("log(Number of seeds per reproductive unit)")


#Seedprotein

names(alltraits2)[names(alltraits2)=="Seed protein content per mass"] <-"Seedprotein" #ddply doesn't seem to like spaces in column names, or ""ing them

cSeedprotein<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(Seedprotein)),
             mean = mean(Seedprotein, na.rm=TRUE),
             sd = sd(Seedprotein, na.rm=TRUE),
             se = sd/sqrt(N))


ggplot(cSeedprotein, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2)  + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Seed protein content per mass (%)")
...
ggplot(cseed, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2)  + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Seed mass (mg)")

#RGR

names(alltraits2)[names(alltraits2)=="Plant relative growth rate (RGR)"] <-"RGR" #ddply doesn't seem to like spaces in column names, or ""ing them

cRGR<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(RGR)),
             mean = mean(RGR, na.rm=TRUE),
             sd = sd(RGR, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cRGR, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Relative growth rate (g/g/day)")

#Photosynthrate
names(alltraits2)[names(alltraits2)=="Photosynthesis per leaf dry mass"] <-"phot" #ddply doesn't seem to like spaces in column names, or ""ing them

cphot<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(phot)),
             mean = mean(phot, na.rm=TRUE),
             sd = sd(phot, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cphot, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Photosynthesis (nmol CO2 per g dry leaf per s)")


#Leafresp
names(alltraits2)[names(alltraits2)=="Leaf respiration per dry mass"] <-"resp" #ddply doesn't seem to like spaces in column names, or ""ing them

cresp<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(resp)),
             mean = mean(resp, na.rm=TRUE),
             sd = sd(resp, na.rm=TRUE),
             se = sd/sqrt(N))

ggplot(cresp, aes(x=Origin, y=mean)) + geom_bar(position=position_dodge(), stat="identity") + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA))+ ylab("Leaf respiration (nmol CO2 per g dry leaf per s)")

#boxplots (relies on the renaming of columns done above)

test<-melt(alltraits2)

traits19<-alltraits2[,c(10,11,15,22,23,25,29,30,31,43,49,59,60,62,64,65,73,74,75,77)]

traits19$Origin<-factor(traits19$Origin, levels=c("N", "I"))


#Make each boxplot separately
ggplot(traits19, aes(x=Origin, y=leafC))+geom_boxplot() + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf C per dry mass (mg/g)")

ggplot(traits19, aes(x=Origin, y=leafN))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf N per dry mass (mg/g)")

ggplot(traits19, aes(x=Origin, y=leafP))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf P per dry mass (mg/g)")

ggplot(traits19, aes(x=Origin, y=leafK))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf K per dry mass (mg/g)")

ggplot(traits19, aes(x=Origin, y=leafCN))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf C:N Ratio")

ggplot(traits19, aes(x=Origin, y=leafNP))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf N:P Ratio")

ggplot(traits19, aes(x=Origin, y=log(sla,10)))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("log SLA")

ggplot(traits19, aes(x=Origin, y=lwr))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf weight ratio")

ggplot(traits19, aes(x=Origin, y=stomataconductance))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Stomata conductance")

ggplot(traits19, aes(x=Origin, y=log(ssd, 10)))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Log SSD")

ggplot(traits19, aes(x=Origin, y=woodymass))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Woody dry mass (%)")

ggplot(traits19, aes(x=Origin, y=RootN))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Root N per dry mass (mg/g)")

ggplot(traits19, aes(x=Origin, y=rootdepth))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Rooting depth (cm)")

ggplot(traits19, aes(x=Origin, y=log(seedno,10)))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Log number of seeds per reproductive unit")

ggplot(traits19, aes(x=Origin, y=Seedprotein))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Seed protein per mass (%)")

ggplot(traits19, aes(x=Origin, y=log(Seedmass,10))) +geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Log seed mass (g)")

ggplot(traits19, aes(x=Origin, y=RGR))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Relative growth rate (g/g/day)")

ggplot(traits19, aes(x=Origin, y=phot))+geom_boxplot()+ theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Light-saturated photosynthetic rate (nmol CO2/g dry leaf/s")



#try faceting the boxplots
melted<-melt(traits19)

ggplot(melted,aes(x=Origin, y=value))+geom_boxplot() + facet_wrap(~variable, scales="free")
...
#i shoudl do this again for CWM, for the 2013 data
...
cleafC$Origin<-factor(cleafC$Origin, levels=c("N", "I"))


ggplot(cleafC, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf C per dry mass (mg/g)")
...
cleafN$Origin<-factor(cleafN$Origin, levels=c("N", "I"))

ggplot(cleafN, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf N per dry mass (mg/g)")
...
cleafP$Origin<-factor(cleafP$Origin, levels=c("N", "I"))

ggplot(cleafP, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf P per dry mass (mg/g)")
...
cleafK$Origin<-factor(cleafK$Origin, levels=c("N", "I"))

ggplot(cleafK, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf K per dry mass (mg/g)")
...
cleafCN$Origin<-factor(cleafCN$Origin, levels=c("N", "I"))

ggplot(cleafCN, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf C:N ratio")
...
cleafNP$Origin<-factor(cleafNP$Origin, levels=c("N", "I"))

ggplot(cleafNP, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf N:P ratio")
...
csla$Origin<-factor(csla$Origin, levels=c("N", "I"))

ggplot(csla, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("SLA")

#LWR

names(alltraits2)[names(alltraits2)=="Leaf dry mass per plant dry mass (leaf weight ratio, LWR)"] <-"lwr" #ddply doesn't seem to like spaces in column names, or ""ing them

lwr<-ddply(alltraits2, c("Origin"), summarise,
             N = sum(!is.na(lwr)),
             mean = mean(lwr, na.rm=TRUE),
             sd = sd(lwr, na.rm=TRUE),
             se = sd/sqrt(N),
...
lwr$Origin<-factor(lwr$Origin, levels=c("N", "I"))

ggplot(lwr, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("LWR")
...
cstomataconductance$Origin<-factor(cstomataconductance$Origin, levels=c("N", "I"))

ggplot(cstomataconductance, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Stomata conductance per leaf area")
...
cssd$Origin<-factor(cssd$Origin, levels=c("N", "I"))

ggplot(cssd, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("SSD")
...
cwoodymass$Origin<-factor(cwoodymass$Origin, levels=c("N", "I"))

ggplot(cwoodymass, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Woody dry mass per plant dry mass")
...
crootn$Origin<-factor(crootn$Origin, levels=c("N", "I"))

ggplot(crootN, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Root N (mg per g dry root mass)")
...
crootdepth$Origin<-factor(crootdepth$Origin, levels=c("N", "I"))

ggplot(crootdepth, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Root depth (cm)")
...
ggplot(cseedno, aes(x=Origin, y=log(mean,10))) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=(logmean-logse), ymax = (logmean+logse), width=.2)) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("log(Number of seeds per reproductive unit)")
...
cSeedprotein$Origin<-factor(cSeedprotein$Origin, levels=c("N", "I"))

ggplot(cSeedprotein, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2)  + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Seed protein content per mass (%)")
...
cseed$Origin<-factor(cseed$Origin, levels=c("N", "I"))

ggplot(cseed, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2)  + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Seed mass (mg)")
...
cRGR$Origin<-factor(cRGR$Origin, levels=c("N", "I"))

ggplot(cRGR, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Relative growth rate (g/g/day)")
...
cphot$Origin<-factor(cphot$Origin, levels=c("N", "I"))

ggplot(cphot, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Photosynthesis (nmol CO2 per g dry leaf per s)")
...
cresp$Origin<-factor(cresp$Origin, levels=c("N", "I"))

ggplot(cresp, aes(x=Origin, y=mean)) + geom_point(position=position_dodge(), stat="identity", size=4) + geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width=.2) + theme_bw() + theme (panel.grid.major=element_line(color = NA), panel.grid.minor=element_line(color = NA), text=element_text(size=20))+ ylab("Leaf respiration (nmol CO2 per g dry leaf per s)")
```